...
function Event20000_onUpdate()
    if env(ESD_ENV_DS3GetSpecialEffectID, 10651) == TRUE and (env(ESD_ENV_GetReceivedDamageType) == DAMAGE_TYPE_DEATH or env(ESD_ENV_GetHP) <= 0) then
        Fire("W_Event20004")
        return 
    end
...
function Event20005_onUpdate()
    ...
    if env(ESD_ENV_DS3GetSpecialEffectID, 10650) == TRUE and env(ESD_ENV_HasReceivedAnyDamage) == TRUE then
        Fire("W_Event20000")
        return 
    end
    if TRUE == EventCommonFunction() then
...
function Event30000_onUpdate()
    ...
    if env(ESD_ENV_DS3GetSpecialEffectID, 10652) == TRUE then
        Fire("W_Event20001")
        return 
    end
    if TRUE == EventCommonFunction() then
