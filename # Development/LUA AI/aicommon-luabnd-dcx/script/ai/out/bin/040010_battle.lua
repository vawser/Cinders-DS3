...
Goal.Activate = function (self, ai, goal)
    ...
    if distance >= 6 then
        ...
        actChanceList[5] = 0 -- WA: Heide's Fury
    ...
    if ai:GetHpRate(TARGET_SELF) < 0.25 then
        actChanceList[20] = 20 -- Use Item (Slot 0) - Divine Blessing
    end
    ...
    ----------------------------------
    -- Attacks
    actFuncList[1] = REGIST_FUNC(ai, goal, NPC_Maldron_Act01) -- Right Light Attack + Approach
    actFuncList[2] = REGIST_FUNC(ai, goal, NPC_Maldron_Act02) -- Right Heavy Attack + Approach
    actFuncList[3] = REGIST_FUNC(ai, goal, NPC_Maldron_Act03) -- Kick + Approach
    actFuncList[4] = REGIST_FUNC(ai, goal, NPC_Maldron_Act04) -- Jump Attack + Approach
    actFuncList[5] = REGIST_FUNC(ai, goal, NPC_Maldron_Act05) -- WA: Heide's Fury
    actFuncList[6] = REGIST_FUNC(ai, goal, NPC_Maldron_Act06) -- WA: Shield Bash
    
    -- Utility
    actFuncList[10] = REGIST_FUNC(ai, goal, NPC_Maldron_Act10) -- Approach + Running Attack
    actFuncList[11] = REGIST_FUNC(ai, goal, NPC_Maldron_Act11) -- Backstep Roll
    actFuncList[12] = REGIST_FUNC(ai, goal, NPC_Maldron_Act12) -- Forward Roll + Run + Basic Light Attack
    actFuncList[13] = REGIST_FUNC(ai, goal, NPC_Maldron_Act13) -- Side Roll + Run + Basic Light Attack
    actFuncList[14] = REGIST_FUNC(ai, goal, NPC_Maldron_Act14) -- Back Roll + Basic Light Attack
    actFuncList[15] = REGIST_FUNC(ai, goal, NPC_Maldron_Act15) -- Strafe
    actFuncList[16] = REGIST_FUNC(ai, goal, NPC_Maldron_Act16) -- Backstep
    actFuncList[17] = REGIST_FUNC(ai, goal, NPC_Maldron_Act17) -- Approach
    actFuncList[18] = REGIST_FUNC(ai, goal, NPC_Maldron_Act18) -- Warp to Friend
    
    -- Items
    actFuncList[20] = REGIST_FUNC(ai, goal, NPC_Maldron_Act20)   -- Use Item (Slot 0) - Divine Blessing
...
function NPC_Maldron_Act02(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
...
function NPC_Maldron_Act10(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Backstep Roll
function NPC_Maldron_Act11(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_Maldron_Act12(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_Maldron_Act13(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_Maldron_Act14(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Strafe
function NPC_Maldron_Act15(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    local duration = 1.8
...
-- Backstep
function NPC_Maldron_Act16(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local duration = 1.8
    ...
    elseif ai:IsInterupt(INTERUPT_FindAttack) then
        if distance < 1.8 and roll <= 80 then
            if roll <= 60 and 30 <= stamina then
                goal:ClearSubGoal()
                NPC_Maldron_Act15(ai, goal, paramTbl) -- Strafe
                return true
            elseif stamina <= 35 and 0 <= stamina then
                goal:ClearSubGoal()
                NPC_Maldron_Act12(ai, goal, paramTbl) -- Forward Roll + Run + Basic Light Attack
                return true
            end
        elseif distance <= 3 and 20 <= stamina and roll <= 60 then
            goal:ClearSubGoal()
            NPC_Maldron_Act10(ai, goal, paramTbl) -- Approach + Running Attack
            return true
        end
    -- Occurs if a ranged attack occurs
    elseif ai:IsInterupt(INTERUPT_Shoot) and roll <= 33 and 20 <= stamina then
        goal:ClearSubGoal()
        NPC_Maldron_Act13(ai, goal) -- Side Roll + Run + Basic Light Attack
        return true
    else
        return false
    end
