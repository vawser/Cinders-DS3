RegisterTableGoal(GOAL_NPC_Tiny, "GOAL_NPC_Tiny")
REGISTER_GOAL_NO_SUB_GOAL(GOAL_NPC_Tiny, true)
...
Goal.Activate = function (self, ai, goal)
    ...
    ----------------------------------
    -- Attacks
    actFuncList[1] = REGIST_FUNC(ai, goal, NPC_Tiny_Act01) -- Right Light Attack + Approach
    actFuncList[2] = REGIST_FUNC(ai, goal, NPC_Tiny_Act02) -- Right Heavy Attack + Approach
    actFuncList[3] = REGIST_FUNC(ai, goal, NPC_Tiny_Act03) -- Kick + Approach
    actFuncList[4] = REGIST_FUNC(ai, goal, NPC_Tiny_Act04) -- Jump Attack + Approach
    actFuncList[5] = REGIST_FUNC(ai, goal, NPC_Tiny_Act05) -- WA: Warcry
    
    -- Utility
    actFuncList[10] = REGIST_FUNC(ai, goal, NPC_Tiny_Act10) -- Approach + Running Attack
    actFuncList[11] = REGIST_FUNC(ai, goal, NPC_Tiny_Act11) -- Backstep Roll
    actFuncList[12] = REGIST_FUNC(ai, goal, NPC_Tiny_Act12) -- Forward Roll + Run + Basic Light Attack
    actFuncList[13] = REGIST_FUNC(ai, goal, NPC_Tiny_Act13) -- Side Roll + Run + Basic Light Attack
    actFuncList[14] = REGIST_FUNC(ai, goal, NPC_Tiny_Act14) -- Back Roll + Basic Light Attack
    actFuncList[15] = REGIST_FUNC(ai, goal, NPC_Tiny_Act15) -- Strafe
    actFuncList[16] = REGIST_FUNC(ai, goal, NPC_Tiny_Act16) -- Backstep Walk
    actFuncList[17] = REGIST_FUNC(ai, goal, NPC_Tiny_Act17) -- Approach
    
    -- Items
    actFuncList[20] = REGIST_FUNC(ai, goal, NPC_Tiny_Act20)   -- Use Item (Slot 0) - Dung Pie
    
    Common_Battle_Activate(ai, goal, actChanceList, actFuncList, REGIST_FUNC(ai, goal, NPC_Tiny_ActAfter_AdjustSpace), actTblList)
    return 
end
...
function NPC_Tiny_Act01(self, ai, goal)
...
function NPC_Tiny_Act02(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
...
-- Kick + Approach
function NPC_Tiny_Act03(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
function NPC_Tiny_Act04(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
-- WA: Warcry
function NPC_Tiny_Act05(self, ai, goal)
...
function NPC_Tiny_Act10(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Backstep Roll
function NPC_Tiny_Act11(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_Tiny_Act12(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_Tiny_Act13(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_Tiny_Act14(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Strafe
function NPC_Tiny_Act15(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    local duration = 1.8
...
-- Backstep
function NPC_Tiny_Act16(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local duration = 1.8
...
-- Approach
function NPC_Tiny_Act17(self, ai, goal)
...
function NPC_Tiny_Act20(self, ai, goal)
...
-- Act After
-------------------------
function NPC_Tiny_ActAfter_AdjustSpace(self, ai, goal)
    return 
end
    ...
    elseif ai:IsInterupt(INTERUPT_FindAttack) then
        if distance < 1.8 and roll <= 80 then
            if roll <= 60 and 30 <= stamina then
                goal:ClearSubGoal()
                NPC_Tiny_Act15(ai, goal, paramTbl) -- Strafe
                return true
            elseif stamina <= 35 and 0 <= stamina then
                goal:ClearSubGoal()
                NPC_Tiny_Act12(ai, goal, paramTbl) -- Forward Roll + Run + Basic Light Attack
                return true
            end
        elseif distance <= 3 and 20 <= stamina and roll <= 60 then
            goal:ClearSubGoal()
            NPC_Tiny_Act10(ai, goal, paramTbl) -- Approach + Running Attack
            return true
        end
    -- Occurs if a ranged attack occurs
    elseif ai:IsInterupt(INTERUPT_Shoot) and roll <= 33 and 20 <= stamina then
        goal:ClearSubGoal()
        NPC_Tiny_Act13(ai, goal) -- Side Roll + Run + Basic Light Attack
        return true
    else
        return false
    end
