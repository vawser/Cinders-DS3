RegisterTableGoal(GOAL_NPC_AshenHunter, "GOAL_NPC_AshenHunter")
REGISTER_GOAL_NO_SUB_GOAL(GOAL_NPC_AshenHunter, true)
...
Goal.Activate = function (self, ai, goal)
    ...
    ----------------------------------
    -- Attacks
    actFuncList[1] = REGIST_FUNC(ai, goal, NPC_AshenHunter_Act01) -- Right Light Attack + Approach
    ...
    actFuncList[11] = REGIST_FUNC(ai, goal, NPC_AshenHunter_Act11) -- Backstep Roll
    actFuncList[12] = REGIST_FUNC(ai, goal, NPC_AshenHunter_Act12) -- Forward Roll + Run + Basic Light Attack
    actFuncList[13] = REGIST_FUNC(ai, goal, NPC_AshenHunter_Act13) -- Side Roll + Run + Basic Light Attack
    actFuncList[14] = REGIST_FUNC(ai, goal, NPC_AshenHunter_Act14) -- Back Roll + Basic Light Attack
...
function NPC_AshenHunter_Act01(self, ai, goal)
...
function NPC_AshenHunter_Act02(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
...
-- Kick + Approach
function NPC_AshenHunter_Act03(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
function NPC_AshenHunter_Act04(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
function NPC_AshenHunter_Act10(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Backstep Roll
function NPC_AshenHunter_Act11(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_AshenHunter_Act12(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_AshenHunter_Act13(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_AshenHunter_Act14(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Strafe
function NPC_AshenHunter_Act15(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    local duration = 1.8
...
-- Backstep
function NPC_AshenHunter_Act16(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local duration = 1.8
...
-- Act After
-------------------------
function NPC_AshenHunter_ActAfter_AdjustSpace(self, ai, goal)
    return 
end
    ...
    elseif ai:IsInterupt(INTERUPT_FindAttack) then
        if distance < 1.8 and roll <= 80 then
            if roll <= 60 and 30 <= stamina then
                goal:ClearSubGoal()
                NPC_AshenHunter_Act15(ai, goal, paramTbl) -- Strafe
                return true
            elseif stamina <= 35 and 0 <= stamina then
                goal:ClearSubGoal()
                NPC_AshenHunter_Act12(ai, goal, paramTbl) -- Forward Roll + Run + Basic Light Attack
                return true
            end
        elseif distance <= 3 and 20 <= stamina and roll <= 60 then
            goal:ClearSubGoal()
            NPC_AshenHunter_Act10(ai, goal, paramTbl) -- Approach + Running Attack
            return true
        end
    -- Occurs if a ranged attack occurs
    elseif ai:IsInterupt(INTERUPT_Shoot) and roll <= 33 and 20 <= stamina then
        goal:ClearSubGoal()
        NPC_AshenHunter_Act13(ai, goal) -- Side Roll + Run + Basic Light Attack
        return true
    else
        return false
    end
