RegisterTableGoal(GOAL_NPC_BlacksmithNightmare, "GOAL_NPC_BlacksmithNightmare")
REGISTER_GOAL_NO_SUB_GOAL(GOAL_NPC_BlacksmithNightmare, true)
...
Goal.Activate = function (self, ai, goal)
    ...
    if distance >= 6 then
        ...
        actChanceList[5] = 0 -- WA: Smelter's Fury
        ...
        actChanceList[30] = 5 -- Cast Spell (Slot 0) - Chaos Storm
        actChanceList[31] = 0 -- Cast Spell (Slot 1) - Incendiary Whip
    elseif distance >= 3 then
        ...
        actChanceList[5] = 5 -- WA: Smelter's Fury
        ...
        actChanceList[20] = 10 -- Use Item (Slot 0) - Black Firebomb
        ...
        actChanceList[5] = 15 -- WA: Smelter's Fury
        ...
        actChanceList[30] = 5 -- Cast Spell (Slot 0) - Chaos Storm
        actChanceList[31] = 10 -- Cast Spell (Slot 1) - Incendiary Whip
    end
    ...
    -- Block repeat usage of WA while active
    ai:AddObserveSpecialEffectAttribute(TARGET_SELF, 130069500)
    
    if ai:HasSpecialEffectId(TARGET_SELF, 130069500) then
        actChanceList[5] = 0 -- WA: Smelter's Fury
    end
    ...
    ----------------------------------
    -- Attacks
    actFuncList[1] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act01) -- Right Light Attack + Approach
    actFuncList[2] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act02) -- Right Heavy Attack + Approach
    actFuncList[3] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act03) -- Kick + Approach
    actFuncList[4] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act04) -- Jump Attack + Approach
    actFuncList[5] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act05) -- WA: Smelter's Fury
    
    -- Utility
    actFuncList[10] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act10) -- Approach + Running Attack
    actFuncList[11] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act11) -- Backstep Roll
    actFuncList[12] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act12) -- Forward Roll + Run + Basic Light Attack
    actFuncList[13] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act13) -- Side Roll + Run + Basic Light Attack
    actFuncList[14] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act14) -- Back Roll + Basic Light Attack
    actFuncList[15] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act15) -- Strafe
    actFuncList[16] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act16) -- Backstep
    actFuncList[17] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act17) -- Approach
    
    -- Items
    actFuncList[20] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act20)   -- Use Item (Slot 0) - Black Firebomb
    
    -- Spells
    actFuncList[30] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act30) -- Cast Spell (Slot 0) - Chaos Storm
    actFuncList[31] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act31) -- Cast Spell (Slot 1) - Incendiary Whip
    actFuncList[32] = REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_Act32) -- Cast Spell (Slot 2) - Tears of Denial
    
    Common_Battle_Activate(ai, goal, actChanceList, actFuncList, REGIST_FUNC(ai, goal, NPC_BlacksmithNightmare_ActAfter_AdjustSpace), actTblList)
    return 
end
...
function NPC_BlacksmithNightmare_Act01(self, ai, goal)
...
function NPC_BlacksmithNightmare_Act02(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
...
-- Kick + Approach
function NPC_BlacksmithNightmare_Act03(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
function NPC_BlacksmithNightmare_Act04(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
-- WA: Smelter's Fury
function NPC_BlacksmithNightmare_Act05(self, ai, goal)
...
function NPC_BlacksmithNightmare_Act10(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Backstep Roll
function NPC_BlacksmithNightmare_Act11(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_BlacksmithNightmare_Act12(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_BlacksmithNightmare_Act13(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
function NPC_BlacksmithNightmare_Act14(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Strafe
function NPC_BlacksmithNightmare_Act15(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    local duration = 1.8
...
-- Backstep
function NPC_BlacksmithNightmare_Act16(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local duration = 1.8
...
-- Approach
function NPC_BlacksmithNightmare_Act17(self, ai, goal)
...
function NPC_BlacksmithNightmare_Act20(self, ai, goal)
...
-- Cast Spell (Slot 0) - Chaos Storm
function NPC_BlacksmithNightmare_Act30(self, ai, goal)
    ...
    subgoal = subgoal:TimingSetTimer(0, 1.0, UPDATE_SUCCESS)
...
-- Cast Spell (Slot 1) - Incendiary Whip
function NPC_BlacksmithNightmare_Act31(self, ai, goal)
    ...
    -- Cast Spell with Left Light Attack
    local subgoal = ai:AddSubGoal(GOAL_COMMON_AttackTunableSpin, 1, NPC_ATK_L1, TARGET_ENE_0, 999, 0, 0)
    subgoal = subgoal:TimingSetTimer(1, 0.5, UPDATE_SUCCESS)
    subgoal:SetLifeEndSuccess(true)
...
-- Act After
-------------------------
function NPC_BlacksmithNightmare_ActAfter_AdjustSpace(self, ai, goal)
    return 
end
    ...
    elseif ai:IsInterupt(INTERUPT_ParryTiming) then
        if not ai:IsBothHandMode(TARGET_SELF) then
            if distance < 2 and roll <= 50 and 20 <= stamina then
                goal:ClearSubGoal()
                goal:AddSubGoal(GOAL_COMMON_AttackTunableSpin, 0.05, NPC_ATK_L2, TARGET_ENE_0, 999, 0, 0) -- Left WA (Shield Bash)
                return true
            end
        end
    ...
    elseif ai:IsInterupt(INTERUPT_FindAttack) then
        if distance < 1.8 and roll <= 80 then
            if roll <= 60 and 30 <= stamina then
                goal:ClearSubGoal()
                NPC_BlacksmithNightmare_Act15(ai, goal, paramTbl) -- Strafe
                return true
            elseif stamina <= 35 and 0 <= stamina then
                goal:ClearSubGoal()
                NPC_BlacksmithNightmare_Act12(ai, goal, paramTbl) -- Forward Roll + Run + Basic Light Attack
                return true
            end
        elseif distance <= 3 and 20 <= stamina and roll <= 60 then
            goal:ClearSubGoal()
            NPC_BlacksmithNightmare_Act10(ai, goal, paramTbl) -- Approach + Running Attack
            return true
        end
    -- Occurs if a ranged attack occurs
    elseif ai:IsInterupt(INTERUPT_Shoot) and roll <= 33 and 20 <= stamina then
        goal:ClearSubGoal()
        NPC_BlacksmithNightmare_Act13(ai, goal) -- Side Roll + Run + Basic Light Attack
        return true
    else
        return false
    end
