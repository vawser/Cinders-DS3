RegisterTableGoal(GOAL_NPC_Kirk, "GOAL_NPC_Kirk")
REGISTER_GOAL_NO_SUB_GOAL(GOAL_NPC_Kirk, true)
...
Goal.Activate = function (self, ai, goal)
    ...
    if distance >= far_distance then
        ...
        -- Items
        actChanceList[30] = 0 -- Lacerating Knife
    elseif distance >= mid_distance then
        ...
        -- Items
        actChanceList[30] = 10 -- Lacerating Knife
    else
        -- Attacks
        actChanceList[1] = 15 -- R1 Combo
        actChanceList[3] = 5 -- R2 Combo
        ...
        actChanceList[23] = 0 -- Backstep
        actChanceList[24] = 10 -- Forward Roll
        actChanceList[25] = 10 -- Side Roll
        actChanceList[26] = 0 -- Back Roll
        
        -- Items
        actChanceList[30] = 3 -- Lacerating Knife
    end
    ...
    if ai:HasSpecialEffectId(TARGET_SELF, 5111) then
        actChanceList[30] = 0 -- Use Item (Slot 0) - Lacerating Knife
    end
    ...
    -- Boost rolling when HP is low
    if ai:GetHpRate(TARGET_SELF) < 0.5 then
        actChanceList[24] = 30 -- Forward Roll
        actChanceList[25] = 10 -- Side Roll
    end 
    ...
    -- Gestures
    ----------------------------------
    -- Point Down when player is low and not close
    if ai:GetNumber(0) == 0 and ai:GetHpRate(TARGET_ENE_0) <= 0.5 and distance >= 5.0 then
        actChanceList[50] = 500
    end
    ...
    ----------------------------------
    -- Attacks
    actFuncList[1] = REGIST_FUNC(ai, goal, NPC_Kirk_Act01) -- R1 Combo
    actFuncList[3] = REGIST_FUNC(ai, goal, NPC_Kirk_Act03) -- R2 Combo
    actFuncList[4] = REGIST_FUNC(ai, goal, NPC_Kirk_Act04) -- Kick
    actFuncList[5] = REGIST_FUNC(ai, goal, NPC_Kirk_Act05) -- Jump Attack
    actFuncList[6] = REGIST_FUNC(ai, goal, NPC_Kirk_Act06) -- Running R1/R2
    actFuncList[7] = REGIST_FUNC(ai, goal, NPC_Kirk_Act07) -- Right WA: Stance
    
    -- Movement
    actFuncList[20] = REGIST_FUNC(ai, goal, NPC_Kirk_Act20) -- Approach
    actFuncList[21] = REGIST_FUNC(ai, goal, NPC_Kirk_Act21) -- Strafe
    actFuncList[22] = REGIST_FUNC(ai, goal, NPC_Kirk_Act22) -- Move Backwards
    actFuncList[23] = REGIST_FUNC(ai, goal, NPC_Kirk_Act23) -- Backstep
    actFuncList[24] = REGIST_FUNC(ai, goal, NPC_Kirk_Act24) -- Forward Roll
    actFuncList[25] = REGIST_FUNC(ai, goal, NPC_Kirk_Act25) -- Side Roll
    actFuncList[26] = REGIST_FUNC(ai, goal, NPC_Kirk_Act26) -- Back Roll
    
    -- Items
    actFuncList[30] = REGIST_FUNC(ai, goal, NPC_Kirk_Act30) -- Lacerating Knife
    
    -- Gestures
    actFuncList[50] = REGIST_FUNC(ai, goal, NPC_Kirk_Act50) -- Point Down
    
    -- Utility
    actFuncList[60] = REGIST_FUNC(ai, goal, NPC_Kirk_Act60) -- Switch to 1H
    actFuncList[61] = REGIST_FUNC(ai, goal, NPC_Kirk_Act61) -- Switch to 2H
    actFuncList[62] = REGIST_FUNC(ai, goal, NPC_Kirk_Act62) -- Switch Right Hand Primary
    actFuncList[63] = REGIST_FUNC(ai, goal, NPC_Kirk_Act63) -- Switch Right Hand Secondary
    actFuncList[64] = REGIST_FUNC(ai, goal, NPC_Kirk_Act64) -- Switch Left Hand Primary
    actFuncList[65] = REGIST_FUNC(ai, goal, NPC_Kirk_Act65) -- Switch Left Hand Secondary
    
    Common_Battle_Activate(ai, goal, actChanceList, actFuncList, REGIST_FUNC(ai, goal, NPC_Kirk_ActAfter_AdjustSpace), actTblList)
    return 
end
...
--------------------------------------------------
-- R1 Combo
function NPC_Kirk_Act01(self, ai, goal)
...
-- R2 Combo
function NPC_Kirk_Act03(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Kick
function NPC_Kirk_Act04(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Jump Attack
function NPC_Kirk_Act05(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Running R1/R2
function NPC_Kirk_Act06(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    local roll = self:GetRandam_Int(1, 100)
...
-- Right WA: Stance
function NPC_Kirk_Act07(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
--------------------------------------------------
-- Approach
function NPC_Kirk_Act20(self, ai, goal)
...
-- Strafe
function NPC_Kirk_Act21(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Move Backwards
function NPC_Kirk_Act22(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
...
-- Backstep
function NPC_Kirk_Act23(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Forward Roll
function NPC_Kirk_Act24(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    ...
    if ai:GetHpRate(TARGET_SELF) < 0.5 then
        local min_roll_distance = 0.0
        local max_roll_distance = 1.0
    end
...
-- Side Roll
function NPC_Kirk_Act25(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Back Roll
function NPC_Kirk_Act26(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
--------------------------------------------------
-- Lacerating Knife
function NPC_Kirk_Act30(self, ai, goal)
...
--------------------------------------------------
-- Point Down
function NPC_Kirk_Act50(self, ai, goal)
...
--------------------------------------------------
-- Switch to 1H
function NPC_Kirk_Act60(self, ai, goal)
    if self:IsBothHandMode(TARGET_SELF) then
...
-- Switch to 2H
function NPC_Kirk_Act61(self, ai, goal)
    if not self:IsBothHandMode(TARGET_SELF) then
        ai:AddSubGoal(GOAL_COMMON_ComboTunable_SuccessAngle180, 10, NPC_ATK_ButtonTriangle, TARGET_ENE_0, 999, 0, 0) -- Toggle 2H state of Weapon
    end
...
function NPC_Kirk_Act62(self, ai, goal)
...
function NPC_Kirk_Act63(self, ai, goal)
    if self:GetEquipWeaponIndex(ARM_R) == WEP_Primary then
        ai:AddSubGoal(GOAL_COMMON_ComboTunable_SuccessAngle180, 10, NPC_ATK_ArrowKeyRight, TARGET_ENE_0, 999, 0, 0) -- Switch Weapon (Left)
    end
...
function NPC_Kirk_Act64(self, ai, goal)
...
function NPC_Kirk_Act65(self, ai, goal)
    if self:GetEquipWeaponIndex(ARM_L) == WEP_Primary then
        ai:AddSubGoal(GOAL_COMMON_ComboTunable_SuccessAngle180, 10, NPC_ATK_ArrowKeyLeft, TARGET_ENE_0, 999, 0, 0) -- Switch Weapon (Left)
    end
...
-- Act After
-------------------------
function NPC_Kirk_ActAfter_AdjustSpace(self, ai, goal)
    return 
end
...
Goal.Interrupt = function (self, ai, goal)
    ...
    elseif ai:IsInterupt(INTERUPT_FindAttack) then
        ...
        if roll <= avoidance_chance then
            ...
            if distance > avoidance_distance then
                ...
                if roll <= 50 then
                    goal:ClearSubGoal()
                    NPC_Kirk_Act21(ai, goal, paramTbl)
                    return true
                -- Forward Roll
                else 
                    goal:ClearSubGoal()
                    NPC_Kirk_Act24(ai, goal, paramTbl)
                    return true
                end
            -- If attack is close, running attack
                ...
                -- Running R1/R1
                goal:ClearSubGoal()
                NPC_Kirk_Act06(ai, goal, paramTbl)
                return true
            end
        end
    ----------------------------------
    ...
    elseif ai:IsInterupt(INTERUPT_Shoot) then
        ...
        if roll <= avoidance_chance then 
            goal:ClearSubGoal()
            NPC_Kirk_Act25(ai, goal) -- Side Roll
            return true
        end
    else
        return false
